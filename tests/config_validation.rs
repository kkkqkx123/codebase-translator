...
fn validate_configs() -> ConfigValidationResult {
    ...
    let global_config = if global_config_path.exists() {
        ...
        result.errors.push("Global config file not found".to_string());
        None
    };
    ...
    let project_config = if project_config_path.exists() {
        ...
        result.errors.push("Project config file not found".to_string());
        None
    };
    ...
    if let Some(ref config) = global_config {
        ...
        result.deeplx_api_key_configured = config.deeplx.api_key.as_ref()
        ...
        result.provider_availabilities.push(ProviderAvailability {
            ...
            reason: if deeplx_available { None } else { Some("未在 enabled_providers 中启用".to_string()) },
        });

        println!("\n[DeepLX]");
        println!("  启用状态: {}", if result.deeplx_configured { "✓ 已启用" } else { "✗ 未启用" });
        println!("  可用状态: {}", if deeplx_available { "✓ 可用" } else { "✗ 不可用" });
        println!("  API Key: {}", if result.deeplx_api_key_configured { "已配置" } else { "未配置" });

        // LLM 配置检查
        result.llm_configured = result.enabled_providers.contains(&"llm".to_string());
        println!("\n[LLM]");
        println!("  启用状态: {}", if result.llm_configured { "✓ 已启用" } else { "✗ 未启用" });
        ...
        for provider in &config.llm.providers {
            ...
            result.llm_api_keys_configured.push((provider.id.clone(), has_valid_key));
            
            let model_count = if provider.model_list.is_empty() { 1 } else { provider.model_list.len() };
            let is_multi_model = model_count > 1;
            
            println!("\n  供应商 '{}' ({}):", provider.name, provider.id);
            println!("    - 可用状态: {}", if is_available { "✓ 可用" } else { "✗ 不可用" });
            println!("    - API Key: {}", if has_valid_key { "已配置" } else { "未配置" });
            println!("    - 模型数量: {} {}", 
                model_count,
                if is_multi_model { "(多模型轮询)" } else { "" }
            );
            if is_multi_model {
                println!("    - 模型列表:");
        ...
        let secret_id_ok = config.tencent.secret_id.as_ref()
            .map(|s| !s.is_empty() && !s.starts_with("${"))
            .unwrap_or(false);
        let secret_key_ok = config.tencent.secret_key.as_ref()
            .map(|s| !s.is_empty() && !s.starts_with("${"))
            .unwrap_or(false);
        ...
        println!("\n[Tencent Cloud]");
        println!("  启用状态: {}", if result.tencent_configured { "✓ 已启用" } else { "✗ 未启用" });
        println!("  可用状态: {}", if tencent_available { "✓ 可用" } else { "✗ 不可用" });
        println!("  Secret ID: {}", if secret_id_ok { "已配置" } else { "未配置" });
        println!("  Secret Key: {}", if secret_key_ok { "已配置" } else { "未配置" });
    ...
    if let Some(ref config) = project_config {
        ...
        println!("缓存启用: {}", if config.cache.enabled { "是" } else { "否" });
        println!("缓存目录: {}", config.cache.directory);
        println!("Writer 备份: {}", if config.writer.backup { "是" } else { "否" });
        println!("Writer 预览模式: {}", if config.writer.dry_run { "是" } else { "否" });
...
fn build_global_config_summary(config: &GlobalConfig, result: &ConfigValidationResult) -> GlobalConfigSummary {
    // 计算各 provider 的可用性
    let deeplx_available = result.provider_availabilities.iter()
        .find(|p| p.provider_type == "deeplx")
        .map(|p| p.available)
        .unwrap_or(false);
    
    let llm_available = result.provider_availabilities.iter()
        .find(|p| p.provider_type == "llm")
        .map(|p| p.available)
        .unwrap_or(false);
    
    let tencent_available = result.provider_availabilities.iter()
        .find(|p| p.provider_type == "tencent")
        .map(|p| p.available)
        .unwrap_or(false);

    let available_llm_providers: Vec<LLMProviderSummary> = config.llm.providers.iter().map(|p| {
    ...
    let available_llm_count = available_llm_providers.iter().filter(|p| p.available).count();
    ...
    GlobalConfigSummary {
        ...
        deeplx: DeepLXSummary {
            ...
            api_key_configured: config.deeplx.api_key.as_ref()
                .map(|k| !k.is_empty() && !k.starts_with("${"))
                .unwrap_or(false),
        ...
        tencent: TencentSummary {
            secret_id_configured: config.tencent.secret_id.as_ref()
                .map(|s| !s.is_empty() && !s.starts_with("${"))
                .unwrap_or(false),
            secret_key_configured: config.tencent.secret_key.as_ref()
                .map(|s| !s.is_empty() && !s.starts_with("${"))
                .unwrap_or(false),
...
fn write_validation_report(output_dir: &PathBuf, result: &ConfigValidationResult) {
    ...
    report.push_str("----------------------------------------\n");
    report.push_str(&format!("环境变量文件: {}\n", if result.env_file_loaded { "✓ 已加载" } else { "✗ 未加载" }));
    report.push_str(&format!("全局配置: {}\n", if result.global_config_loaded { "✓ 已加载" } else { "✗ 未加载" }));
    report.push_str(&format!("项目配置: {}\n", if result.project_config_loaded { "✓ 已加载" } else { "✗ 未加载" }));
    report.push('\n');
    ...
    for provider in &result.provider_availabilities {
        ...
        report.push_str(&format!("  启用状态: {}\n", if provider.enabled { "✓ 已启用" } else { "✗ 未启用" }));
        report.push_str(&format!("  可用状态: {}\n", if provider.available { "✓ 可用" } else { "✗ 不可用" }));
    ...
    if result.global_config_loaded {
        ...
        report.push_str(&format!("  已启用: {}\n", if result.deeplx_configured { "是" } else { "否" }));
        report.push_str(&format!("  API Key 已配置: {}\n\n", if result.deeplx_api_key_configured { "是" } else { "否" }));
        ...
        for (provider_id, is_configured) in &result.llm_api_keys_configured {
            report.push_str(&format!("    - {}: {}\n", provider_id, if *is_configured { "已配置" } else { "未配置" }));
        }
        report.push('\n');

        // Tencent
        report.push_str("[Tencent Cloud]\n");
        report.push_str(&format!("  已启用: {}\n", if result.tencent_configured { "是" } else { "否" }));
        report.push_str(&format!("  Secret ID 已配置: {}\n",
            if result.tencent_credentials_configured.0 { "是" } else { "否" }));
        report.push_str(&format!("  Secret Key 已配置: {}\n\n",
            if result.tencent_credentials_configured.1 { "是" } else { "否" }));
    }
    ...
    if let Some(ref details) = result.global_config_details {
        ...
        report.push_str(&format!("  启用状态: {}\n", if details.deeplx.enabled { "已启用" } else { "未启用" }));
        report.push_str(&format!("  可用状态: {}\n", if details.deeplx.available { "可用" } else { "不可用" }));
        report.push_str(&format!("  API URL: {}\n", details.deeplx.api_url));
        report.push_str(&format!("  API Key: {}\n", if details.deeplx.api_key_configured { "已配置" } else { "未配置" }));
        report.push_str(&format!("  Proxy: {}\n", if details.deeplx.proxy_configured { "已配置" } else { "未配置" }));
        report.push_str(&format!("  速率限制: {} 请求/秒\n", details.deeplx.rate_limit));
        report.push_str(&format!("  最大重试: {} 次\n\n", details.deeplx.max_retries));

        report.push_str("[LLM 配置]\n");
        report.push_str(&format!("  启用状态: {}\n", if details.llm.enabled { "已启用" } else { "未启用" }));
        report.push_str(&format!("  可用状态: {}\n", if details.llm.available { "可用" } else { "不可用" }));
        report.push_str(&format!("  健康检查: {}\n", if details.llm.health_check_enabled { "启用" } else { "禁用" }));
        report.push_str(&format!("  供应商总数: {}\n", details.llm.provider_count));
        report.push_str(&format!("  可用供应商: {}/{}\n", details.llm.available_providers_count, details.llm.provider_count));
        ...
        for provider in &details.llm.providers {
            ...
            if provider.is_multi_model {
                report.push_str(&format!("    - 模型模式: 多模型轮询 (共 {} 个模型)\n", provider.model_list.len()));
                report.push_str("    - 模型列表:\n");
                for (i, model) in provider.model_list.iter().enumerate() {
                    let is_primary = i == 0 && provider.model.is_empty() || model == &provider.model;
                    report.push_str(&format!("      {}. {}{}\n", 
                        i + 1, 
                        model,
            ...
            report.push_str(&format!("    - Proxy: {}\n", if provider.proxy_configured { "已配置" } else { "未配置" }));
        ...
        report.push_str(&format!("  启用状态: {}\n", if details.tencent.enabled { "已启用" } else { "未启用" }));
        report.push_str(&format!("  可用状态: {}\n", if details.tencent.available { "可用" } else { "不可用" }));
        report.push_str(&format!("  Secret ID: {}\n", if details.tencent.secret_id_configured { "已配置" } else { "未配置" }));
        report.push_str(&format!("  Secret Key: {}\n", if details.tencent.secret_key_configured { "已配置" } else { "未配置" }));
        report.push_str(&format!("  区域: {}\n", details.tencent.region));
        report.push_str(&format!("  Proxy: {}\n", if details.tencent.proxy_configured { "已配置" } else { "未配置" }));
    ...
    if let Some(ref details) = result.project_config_details {
        ...
        report.push_str(&format!("  预览模式: {}\n\n", if details.writer_dry_run { "是" } else { "否" }));

        report.push_str("[提取配置]\n");
        report.push_str(&format!("  注释: {}\n", if details.extraction_comments { "是" } else { "否" }));
        report.push_str(&format!("  文档字符串: {}\n", if details.extraction_doc_strings { "是" } else { "否" }));
        report.push_str(&format!("  错误消息: {}\n", if details.extraction_error_messages { "是" } else { "否" }));
        report.push_str(&format!("  格式化字符串: {}\n\n", if details.extraction_format_strings { "是" } else { "否" }));
    }
...
fn write_detailed_config(output_dir: &PathBuf, result: &ConfigValidationResult) {
    ...
    json_content.push_str(&format!("    \"env_file_loaded\": {},\n", result.env_file_loaded));
    json_content.push_str(&format!("    \"global_config_loaded\": {},\n", result.global_config_loaded));
    json_content.push_str(&format!("    \"project_config_loaded\": {}\n", result.project_config_loaded));
    json_content.push_str("  },\n");
    ...
    for (i, provider) in result.provider_availabilities.iter().enumerate() {
        json_content.push_str("    {\n");
        json_content.push_str(&format!("      \"provider_type\": \"{}\",\n", provider.provider_type));
    ...
    if let Some(ref details) = result.global_config_details {
        ...
        json_content.push_str(&format!("      \"health_check_enabled\": {},\n", details.llm.health_check_enabled));
        json_content.push_str(&format!("      \"provider_count\": {},\n", details.llm.provider_count));
        json_content.push_str(&format!("      \"available_providers_count\": {},\n", details.llm.available_providers_count));
        ...
        for (i, provider) in details.llm.providers.iter().enumerate() {
            ...
            json_content.push_str(&format!("          \"available\": {},\n", provider.available));
            if let Some(ref reason) = provider.unavailable_reason {
                json_content.push_str(&format!("          \"unavailable_reason\": \"{}\",\n", reason));
            }
            json_content.push_str(&format!("          \"is_multi_model\": {},\n", provider.is_multi_model));
        ...
        json_content.push_str(&format!("      \"secret_id_configured\": {},\n", details.tencent.secret_id_configured));
        json_content.push_str(&format!("      \"secret_key_configured\": {},\n", details.tencent.secret_key_configured));
        json_content.push_str(&format!("      \"region\": \"{}\",\n", details.tencent.region));
        ...
        json_content.push_str(&format!("      \"level\": \"{}\",\n", details.logging.level));
        json_content.push_str(&format!("      \"format\": \"{}\",\n", details.logging.format));
        json_content.push_str(&format!("      \"output\": \"{}\"\n", details.logging.output));
        json_content.push_str("    }\n");
        json_content.push_str("  },\n");
    }
    ...
    if let Some(ref details) = result.project_config_details {
        ...
        json_content.push_str(&format!("    \"source_langs\": {:?},\n", details.source_langs));
        json_content.push_str(&format!("    \"target_lang\": \"{}\",\n", details.target_lang));
        json_content.push_str(&format!("    \"provider\": \"{}\",\n", details.provider));
        json_content.push_str(&format!("    \"include_patterns_count\": {},\n", details.include_patterns_count));
        json_content.push_str(&format!("    \"exclude_patterns_count\": {},\n", details.exclude_patterns_count));
        json_content.push_str(&format!("    \"cache_enabled\": {},\n", details.cache_enabled));
        json_content.push_str(&format!("    \"cache_directory\": \"{}\",\n", details.cache_directory));
        json_content.push_str(&format!("    \"writer_backup\": {},\n", details.writer_backup));
        json_content.push_str(&format!("    \"writer_dry_run\": {},\n", details.writer_dry_run));
        json_content.push_str(&format!("    \"extraction_comments\": {},\n", details.extraction_comments));
        json_content.push_str(&format!("    \"extraction_doc_strings\": {},\n", details.extraction_doc_strings));
        json_content.push_str(&format!("    \"extraction_error_messages\": {},\n", details.extraction_error_messages));
        json_content.push_str(&format!("    \"extraction_format_strings\": {}\n", details.extraction_format_strings));
        json_content.push_str("  }\n");
    }
