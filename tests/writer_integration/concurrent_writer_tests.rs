...
async fn test_concurrent_writer_basic() {
    ...
    for i in 0..3 {
        ...
        let file = create_test_file(&temp_path, &format!("test_{}.txt", i), &content).await;
        files.push(file);
    }
    ...
    for file in &files {
        let content = read_file_content(&file.path).await;
        assert!(
            content.contains("文件"),
...
async fn test_concurrent_writer_with_different_concurrency() {
    ...
    for i in 0..file_count {
        ...
        let file = create_test_file(&temp_path, &format!("concurrent_{}.txt", i), &content).await;
        files.push(file);
    }
...
async fn test_concurrent_writer_with_backup() {
    ...
    let writer = ConcurrentWriter::with_project_path(config, 1, temp_path.clone());
...
async fn test_concurrent_writer_mixed_success_failure() {
    ...
    let file1 = create_test_file(&temp_path, "test_valid.txt", content1).await;

    let content2 = "Another valid content\nLine 2";
    let file2 = create_test_file(&temp_path, "test_valid2.txt", content2).await;
...
async fn test_concurrent_writer_streaming() {
    ...
    tokio::spawn(async move {
        for i in 0..3 {
            ...
            let file = create_test_file(&temp_path, &format!("stream_{}.txt", i), &content).await;
...
async fn test_concurrent_writer_large_batch() {
    ...
    for i in 0..file_count {
        ...
        let file = create_test_file(&temp_path, &format!("batch_{}.txt", i), &content).await;
