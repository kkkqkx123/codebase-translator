...
fn test_reporter_integration_with_workflow() -> Result<()> {
    ...
    stats.total_units = 12;
    stats.translated_units = 12;
    ...
    let report_path = temp_dir_path.join("workflow_report.txt");
    ...
    assert!(
        report_content.contains("Total:      3"),
        "Report should contain total files"
    );
    assert!(
        report_content.contains("Processed:  3"),
        "Report should contain processed files"
    );
    assert!(
        report_content.contains("Total:      12"),
        "Report should contain total units"
    );
    assert!(
        report_content.contains("Translated: 12"),
        "Report should contain translated units"
    );
    assert!(
