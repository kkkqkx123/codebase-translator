...
fn test_cache_hit_basic() {
    ...
    assert!(cached_entry.is_translated, "Entry should be marked as translated");
    assert!(cached_entry.is_valid(123456), "Entry should be valid with same modification time");
}
...
fn test_cache_hit_translation_status() {
    ...
    let retrieved = cache.get(&file_hash).unwrap().unwrap();
    assert!(retrieved.is_translated, "Entry should be marked as translated");
    assert!(retrieved.translation_timestamp > 0, "Translation timestamp should be set");
}
...
fn test_cache_miss_scenarios() {
    ...
    assert_ne!(fingerprint1, fingerprint2, "Different projects should have different fingerprints");
    
    // Should not retrieve entry from different project
    let result = cache2.get(&file_hash).unwrap();
    assert!(result.is_none(), "Should not retrieve entry from different project");
}
...
fn test_cache_hit_with_modified_time() {
    ...
    assert!(cached_entry.is_valid(123456), "Should be valid with same mtime");
    
    // Cache miss with different modification time (file was modified)
    assert!(!cached_entry.is_valid(999999), "Should be invalid with different mtime");
}
...
fn test_cache_hit_persistence() {
    ...
    assert!(cached_entry.is_some(), "Cache should persist across instances");
    
    let cached_entry = cached_entry.unwrap();
    assert!(cached_entry.is_translated, "Translation status should persist");
...
fn test_multiple_cache_hits() {
    ...
    for (file_hash, original_entry) in &entries {
        let cached_entry = cache.get(file_hash).unwrap();
        assert!(cached_entry.is_some(), "Should retrieve entry for {}", file_hash);
...
fn test_cache_invalidation_on_file_modification() {
    ...
    // Cache hit with same modification time
    let cached_entry = cache.get(&file_hash).unwrap().unwrap();
    assert!(cached_entry.is_valid(1000), "Should be valid with same mtime");
    
    // File is modified, new modification time 2000
    // Cache should be considered invalid
    assert!(!cached_entry.is_valid(2000), "Should be invalid after file modification");
    ...
    assert!(new_cached_entry.is_valid(2000), "Should be valid with updated mtime");
    assert!(!new_cached_entry.is_valid(1000), "Should be invalid with old mtime");
}
...
fn test_cache_hit_requires_valid_and_translated() {
    ...
    let is_cache_hit = cached_entry.is_valid(1000) && cached_entry.is_translated;
    assert!(is_cache_hit, "Should be cache hit when valid and translated");
}
