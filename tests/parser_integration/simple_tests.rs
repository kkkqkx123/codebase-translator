...
fn write_units_to_file(filename: &str, units: &[TranslationUnit]) {
    ...
    output.push_str(&format!("Extracted {} translation units from {}\n", units.len(), filename));
    output.push_str("==================================================\n\n");
...
fn test_parse_javascript_file() {
    ...
    assert!(!units.is_empty(), "Should extract units from JavaScript file");
}
...
fn test_rust_doc_comments_extraction() {
    ...
    let doc_units: Vec<_> = units.iter()
        .filter(|u| matches!(u.node_type, codebase_translate::core::models::NodeType::DocString))
        .collect();
...
fn test_rust_macro_extraction() {
    ...
    assert!(has_error_or_log || !units.is_empty(), "Should extract macro strings or other content");
}
...
fn test_rust_string_literals_extraction() {
    ...
    let has_error_strings = units.iter().any(|u| {
        matches!(u.node_type, codebase_translate::core::models::NodeType::ErrorMessage)
    });
    ...
    println!("Rust string literals - Errors: {}, Logs: {}, Formats: {}, Literals: {}",
        has_error_strings, has_log_strings, has_format_strings, has_string_literals);
    
    assert!(!units.is_empty(), "Should extract string literals from Rust file");
}
...
fn test_python_string_literals_extraction() {
    ...
    assert!(!units.is_empty(), "Should extract string literals from Python file");
}
...
fn test_javascript_string_literals_extraction() {
    ...
    assert!(!units.is_empty(), "Should extract string literals from JavaScript file");
}
...
fn test_go_string_literals_extraction() {
    ...
    assert!(!units.is_empty(), "Should extract string literals from Go file");
}
...
fn test_language_filter_zh_to_en_rust() {
    ...
    println!("Language filter analysis - Chinese: {}, English: {}, Other: {}",
    ...
    assert!(!units.is_empty(), "Should extract content from mixed language file");
}
...
fn test_language_filter_zh_to_en_markdown() {
    ...
    println!("Markdown language filter - Chinese-only: {}, English-only: {}, Mixed: {}",
        chinese_units, english_only_units, mixed_units);
    
    assert!(!units.is_empty(), "Should extract content from mixed language markdown");
}
...
fn test_identify_english_only_content() {
    ...
    for (content, expected_ascii, expected_chinese) in test_cases {
        ...
        assert_eq!(is_ascii, expected_ascii, 
            "ASCII check failed for: {}", content);
        assert_eq!(has_chinese, expected_chinese,
            "Chinese check failed for: {}", content);
    }
}
...
fn test_content_filtering_simulation() {
    ...
    let chinese_units: Vec<_> = units.iter()
        .filter(|u| contains_chinese(&u.content))
        .collect();
    
    let english_only_units: Vec<_> = units.iter()
        .filter(|u| is_ascii_only(&u.content))
        .collect();
    ...
    assert!(!chinese_units.is_empty(), "Should find Chinese content to translate");
    ...
    for (i, unit) in chinese_units.iter().enumerate() {
        output.push_str(&format!("{}: [{}] {}\n", i + 1, unit.node_type, unit.content));
    }
    ...
    println!("Filtered output written to: {}", filtered_output_path.display());
}
