...
fn test_load_project_config() {
    ...
    if !fixture_config.exists() {
        println!("Skipping test: fixture config not found at {:?}", fixture_config);
        return;
    }
    
    let loader = ConfigLoader::new().with_project_config(&fixture_config);
    let config = loader.load_project().expect("Failed to load project config");
...
fn test_e2e_translation_rust_file() {
    ...
    match result {
        Ok(workflow_result) => {
            ...
            output.push_str(&format!("Translated units: {}\n", workflow_result.stats.translated_units));
            output.push_str(&format!("Cached units: {}\n", workflow_result.stats.cached_units));
            output.push_str(&format!("Skipped units: {}\n", workflow_result.stats.skipped_units));
